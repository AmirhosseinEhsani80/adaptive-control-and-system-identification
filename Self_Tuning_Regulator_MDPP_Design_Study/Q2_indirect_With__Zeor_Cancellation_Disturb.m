%% Amirhossein Ehsani - 810602159 - Q2_Disturbance with out Integrator
clc
clear all 
close all
...
R_solved = [1,-1.6,0.70]; 
S_solved = [-2.5,4.3,-1.99];
T = [-0,0,0];
Ao = [1];

% Real R, S, and T
R_real = [1.0000   -1.2480    0.2483];
S_real = [ 5.7498   -7.9550    2.7077];
T_real = [9.3926  -14.2578    5.3465];
...
for i = skip_instances:length(uc)
    ...
    u(i) = T * [uc(i:-1:i-(length(T)-1))].' + S_solved * [-y(i:-1:i-(length(S_solved)-1))].' - R_solved(2:end) * [u_toPlot(i-1:-1:i-(length(R_solved)-1))].' + v(i);
