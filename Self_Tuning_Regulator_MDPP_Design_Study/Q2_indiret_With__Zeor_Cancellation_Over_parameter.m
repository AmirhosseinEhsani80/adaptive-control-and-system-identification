%% Amirhossein Ehsani - 810602159 - Q2_Over Parameter
clc
clear all 
close all
...
% Initial conditions
A_estimated = [1,0,0,0,0]; % Initial A coefficients
B_estimated = [-0.3, 0.5, -0.2,0.12]; % Initial B coefficients
...
R_estimated = [1,-2.5,-0.5,0]; 
S_estimated = [-76,13.4,-5.6,0];
T = [1.1,0,0,0];
Ao = [1];

% Real R, S, and T
R_real = [1.0000   -0.4992   -0.6136    0.2010];
S_real = [1.9118    2.0816   -5.4748    2.1456];
T_real = [1.2027   -0.7308   -0.0144         0];
...
% R Parameters
total_plot_rows = ceil(length(R_estimated_toPlot(1,:))/2) + ceil(length(S_estimated_toPlot(1,:))/2) + ceil(length(T_toPlot(1,:))/2) ;
f2 = figure();
