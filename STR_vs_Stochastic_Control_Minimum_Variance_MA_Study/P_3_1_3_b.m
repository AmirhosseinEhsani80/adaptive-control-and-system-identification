%% Amirhossein Ehsani_810602159_ Sim 3_ Q1_DirSTR _With White Noise
clc;
clear all;
close all;
...
inputNoiseVariance = 0.001;
inputNoise = sqrt(inputNoiseVariance) * randn(1, numberOfSamples);
controlSignal(1) = -1;
controlSignal = controlSignal + inputNoise;

% Noise and its degree
noisePolynomial = [1, 0, 0].';
degreeNoise = length(noisePolynomial);
systemNoiseVariance = 0.1; % System noise variance
...
R_real = [1.2215, -0.8471];
S_real = [1.0145, 0.0938];
T_real = [1.4535, -0.1366];
...
for i = skipInstances:numberOfSamples
    ...
    noise_t = [systemNoise(i:-1:i-(degreeNoise-1))] * noisePolynomial; % Only for simulation, not involved in controller calculations directly
...
for i = 1:4
    ...
    hold on;
    plot(1:numberOfSamples, ones(numberOfSamples, 1) * theta_hat_toPlot(1000, i), '--', 'LineWidth', 1.5, 'Color', [0.9 0.2 0.2]);
