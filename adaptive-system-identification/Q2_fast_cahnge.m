%% Simulation_1 Q_2_fast_change_Params Amirhossein Ehsani_810602159
% RLS SA PA LMS 
...
% Figure containing subplots of figure 1 and 3
figure(1);
subplot(2,1,1);
hold on; 
for i = 1:12
    plot(1:N, Teta_rls_landa(i,:),'Color',colors(i,:) ,'LineWidth', 1.5, 'DisplayName', sprintf('%s%d', char('a'*(i<=6) + 'b'*(i>6)), mod(i-1,6)+1));
    plot(1:N,pt(i,:)','Color',colors(i,:), 'LineWidth', 1.5, 'LineStyle', '--');
end
...
subplot(2,1,2);
hold on;
plot(1:N, Teta_rls_landa(9,:), 'Color',colors(9,:),'LineWidth', 1.5, 'DisplayName', 'b3');
plot(1:N,pt(9,:)', 'Color',colors(9,:),'LineWidth', 1.5,'LineStyle', '--', 'DisplayName', 'True b3');
plot(1:N, Teta_rls_landa(10,:), 'Color',colors(10,:),'LineWidth', 1.5, 'DisplayName', 'b4');
plot(1:N,pt(10,:)', 'Color',colors(10,:),'LineWidth', 1.5,'LineStyle', '--', 'DisplayName', 'True b4');
title('Detailed View of Parameters b3 and b4 from \lambda-RLS');
legend('show', 'Location', 'best');
xlabel('Sample');
ylabel('Parameter Value');
grid on;
...
% Figure containing subplots of figure 2 and 4
figure(2);
subplot(2,1,1);
hold on; 
for i = 1:12
    plot(1:N, Teta_rls_p(i,:),'Color',colors(i,:) ,'LineWidth', 1.5, 'DisplayName', sprintf('%s%d', char('a'*(i<=6) + 'b'*(i>6)), mod(i-1,6)+1));
    plot(1:N,pt(i,:)','Color',colors(i,:), 'LineWidth', 1.5, 'LineStyle', '--');
end
...
subplot(2,1,2);
hold on;
plot(1:N, Teta_rls_p(9,:), 'Color',colors(9,:),'LineWidth', 1.5, 'DisplayName', 'b3');
plot(1:N,pt(9,:)', 'Color',colors(9,:),'LineWidth', 1.5,'LineStyle', '--', 'DisplayName', 'True b3');
plot(1:N, Teta_rls_p(10,:), 'Color',colors(10,:),'LineWidth', 1.5, 'DisplayName', 'b4');
plot(1:N,pt(10,:)', 'Color',colors(10,:),'LineWidth', 1.5,'LineStyle', '--', 'DisplayName', 'True b4');
title('Detailed View of Parameters b3 and b4 from P-Reset-RLS');
legend('show', 'Location', 'best');
xlabel('Sample');
ylabel('Parameter Value');
grid on;
...
method.b1 =[b1;Teta_rls_landa(4,N);Teta_rls_p(4,N)];
method.b2 =[b2;Teta_rls_landa(5,N);Teta_rls_p(5,N)];
method.b3 =[b1;Teta_rls_landa(4,N);Teta_rls_p(4,N)];
method.b4 =[b2;Teta_rls_landa(5,N);Teta_rls_p(5,N)];
method.b5 =[b1;Teta_rls_landa(4,N);Teta_rls_p(4,N)];
method.b6 =[b2;Teta_rls_landa(5,N);Teta_rls_p(5,N)];
